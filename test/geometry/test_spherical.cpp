...
TEST(SphericalPointTest, ExtensiveRoundTripTests) {
    ...
    std::vector<Point<Float, 2>> test_points_2d = {
        Point<Float, 2>(1, 0),       Point<Float, 2>(0, 1),      Point<Float, 2>(-1, 0), Point<Float, 2>(0, -1),
        Point<Float, 2>(1, 1),       Point<Float, 2>(-1, -1),    Point<Float, 2>(2, 3),  Point<Float, 2>(-1.5f, 2.5f),
    ...
    // 3D test cases
    std::vector<Point<Float, 3>> test_points_3d = {Point<Float, 3>(1, 0, 0),
                                                   Point<Float, 3>(0, 1, 0),
                                                   Point<Float, 3>(0, 0, 1),
                                                   Point<Float, 3>(-1, 0, 0),
                                                   Point<Float, 3>(0, -1, 0),
...
TEST(SphericalGeometryTest, SphericalPolygonAreaBasic) {
    ...
    std::vector<Vector<Float, 3>> triangle_vertices = {Vector<Float, 3>(1, 0, 0).normalized(),
...
TEST(SphericalGeometryTest, ErrorHandling) {
    ...
    std::vector<Vector<Float, 3>> min_vertices = {Vector<Float, 3>(1, 0, 0).normalized(),
                                                  Vector<Float, 3>(0, 1, 0).normalized(),
                                                  Vector<Float, 3>(0, 0, 1).normalized()};
...
TEST(SphericalGeometryTest, SphericalPolygonAreaConsistency) {
    ...
    std::vector<Vector<Float, 3>> quad_vertices = {
        Vector<Float, 3>(1, 0, 0).normalized(), Vector<Float, 3>(Float(0.7071), Float(0.7071), 0).normalized(),
        Vector<Float, 3>(0, 1, 0).normalized(), Vector<Float, 3>(Float(-0.7071), Float(0.7071), 0).normalized()};

    Float polygon_area = spherical_polygon_area(quad_vertices);

    // Manual triangulation: split into two triangles
    Float triangle1_area = spherical_triangle_area(quad_vertices[0], quad_vertices[1], quad_vertices[2]);
    Float triangle2_area = spherical_triangle_area(quad_vertices[0], quad_vertices[2], quad_vertices[3]);
...
TEST(SphericalGeometryTest, SphericalPolygonAreaVertexOrder) {
    ...
    std::vector<Vector<Float, 3>> vertices_ccw = {Vector<Float, 3>(1, 0, 0).normalized(),
                                                  Vector<Float, 3>(0, 1, 0).normalized(),
                                                  Vector<Float, 3>(0, 0, 1).normalized()};

    std::vector<Vector<Float, 3>> vertices_cw = {Vector<Float, 3>(1, 0, 0).normalized(),
                                                 Vector<Float, 3>(0, 0, 1).normalized(),
                                                 Vector<Float, 3>(0, 1, 0).normalized()};
...
TEST(SphericalUtilityTest, SinThetaFunction) {
    ...
    Vector<Float, 3> various_vectors[] = {Vector<Float, 3>(1, 2, 3).normalized(),
...
TEST(SphericalUtilityTest, PhiFunction) {
    ...
    std::vector<Vector<Float, 3>> test_vectors = {Vector<Float, 3>(1, 1, 1), Vector<Float, 3>(-1, 1, -1),
...
TEST(SphericalUtilityTest, TrigonometricIdentities) {
    ...
    std::vector<Vector<Float, 3>> test_vectors = {
        Vector<Float, 3>(1, 0, 0).normalized(),  Vector<Float, 3>(0, 1, 0).normalized(),
        Vector<Float, 3>(0, 0, 1).normalized(),  Vector<Float, 3>(1, 1, 1).normalized(),
        Vector<Float, 3>(2, -3, 4).normalized(), Vector<Float, 3>(-1, 2, -3).normalized()};
...
TEST(SphericalUtilityTest, WrapAngle2PiExtensive) {
    ...
    std::vector<Float> test_angles = {Float(-10), Float(-5.5), Float(-1),  Float(0),   Float(1),
...
TEST(SphericalUtilityTest, WrapAngle2PiConsistency) {
    ...
    std::vector<Float> test_angles = {Float(-7), Float(-3.5), Float(0), Float(3.5), Float(7), Float(15)};
