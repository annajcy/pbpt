...
constexpr math::Float kEpsilon = 1e-6;

// Helpers replacing the removed Homogeneous type.
// Applies a 4x4 matrix to a point (w=1) and performs the projective divide.
// TM = matrix scalar, TP = point scalar (may differ when using Transform<double> with math::Pt3=math::Point<float>).
template <typename TM, typename TP>
static math::Point<TM, 3> mat_apply_point(const math::Matrix<TM, 4, 4>& m, const math::Point<TP, 3>& p) {
    auto h = m * math::Vector<TM, 4>(TM(p.x()), TM(p.y()), TM(p.z()), TM(1));
    const TM inv_w = TM(1) / h[3];
    return math::Point<TM, 3>(h[0] * inv_w, h[1] * inv_w, h[2] * inv_w);
}

// Applies a 4x4 matrix to a direction vector (w=0); w component is discarded.
template <typename TM, typename TV>
static math::Vector<TM, 3> mat_apply_vector(const math::Matrix<TM, 4, 4>& m, const math::Vector<TV, 3>& v) {
    auto h = m * math::Vector<TM, 4>(TM(v.x()), TM(v.y()), TM(v.z()), TM(0));
    return math::Vector<TM, 3>(h[0], h[1], h[2]);
}
...
protected:
    ...
    const math::Pt3 p_x{10.0, 0.0, 0.0};
    // A point on the Y-axis
    const math::Pt3 p_y{0.0, 10.0, 0.0};
    // A vector pointing up
    const math::Vec3 v_up{0.0, 1.0, 0.0};
    // A point at the origin
    const math::Pt3 origin{0.0, 0.0, 0.0};
};

TEST_F(TransformTest, Translate) {
    const math::Vec3 translation_vec(5, -2, 100);
    const auto t = Transform<double>::translate(translation_vec).matrix();

    const auto p_transformed = mat_apply_point(t, origin);
    EXPECT_DOUBLE_EQ(p_transformed.x(), 5.0);
    EXPECT_DOUBLE_EQ(p_transformed.y(), -2.0);
    EXPECT_DOUBLE_EQ(p_transformed.z(), 100.0);

    const auto v_transformed = mat_apply_vector(t, v_up);
...
TEST_F(TransformTest, Scale) {
    const math::Vec3 scale_vec(2, 3, 0.5);
    const auto s = Transform<double>::scale(scale_vec).matrix();

    const math::Pt3 p_start(1, 1, 10);
    const auto p_transformed = mat_apply_point(s, p_start);
    ...
    const math::Vec3 v_start(1, 1, 10);
    const auto v_transformed = mat_apply_vector(s, v_start);
...
TEST_F(TransformTest, RotateZ90Degrees) {
    const auto rz = Transform<double>::rotate_z(math::deg2rad(90.0)).matrix();
    const auto p_transformed = mat_apply_point(rz, p_x);
...
TEST_F(TransformTest, RotateY90Degrees) {
    const auto ry = Transform<double>::rotate_y(math::deg2rad(90.0)).matrix();

    // Rotate a point on the X-axis around Y
    const auto p_transformed = mat_apply_point(ry, p_x);
...
TEST_F(TransformTest, RotateX90Degrees) {
    const auto rx = Transform<double>::rotate_x(math::deg2rad(90.0)).matrix();

    // Rotate a point on the Y-axis around X
    const auto p_transformed = mat_apply_point(rx, p_y);
...
TEST_F(TransformTest, RotateAxisAngle) {
    const auto angle = static_cast<math::Float>(M_PI / 2.0);  // 90 degrees
    const math::Vec3 z_axis(0, 0, 1);

    // An arbitrary axis-angle rotation should be identical to the specific-axis
    // version
    const math::Mat4 r_axis_angle = Transform<double>::rotate(angle, z_axis).matrix();
    const math::Mat4 r_z = Transform<double>::rotate_z(angle).matrix();
...
TEST_F(TransformTest, LookAt) {
    const math::Pt3 eye(0, 0, 10);
    const math::Pt3 target(0, 0, 0);
    const math::Vec3 up(0, 1, 0);
    const auto view_matrix = Transform<double>::look_at(eye, target, up).matrix();

    // A point in the world at (5, 0, 0)
    const math::Pt3 world_point(5, 0, 0);
    const auto view_point = mat_apply_point(view_matrix, world_point);
    ...
    const auto origin_in_view = mat_apply_point(view_matrix, origin);
...
TEST_F(TransformTest, Perspective) {
    ...
    const math::Pt3 near_plane_point(0, 0, z_near);
    const auto near_clip = persp_matrix * math::Vector<double, 4>(double(near_plane_point.x()),
                                                             double(near_plane_point.y()),
                                                             double(near_plane_point.z()), 1.0);
    const double near_w = near_clip[3];
    const auto near_ndc = math::Point<double, 3>(near_clip[0] / near_w, near_clip[1] / near_w, near_clip[2] / near_w);
    ...
    const double far_w = far_clip[3];
    const auto far_ndc = math::Point<double, 3>(far_clip[0] / far_w, far_clip[1] / far_w, far_clip[2] / far_w);
...
TEST_F(TransformTest, Identity) {
    ...
    const math::Pt3 test_point(3.5, -2.1, 7.8);
    const math::Pt3 transformed = identity_transform.transform_point(test_point);
    ...
    const math::Vec3 test_vector(1.2, -4.5, 0.3);
    const math::Vec3 transformed_vec = identity_transform.transform_vector(test_vector);
...
TEST_F(TransformTest, Composition) {
    ...
    const math::Vec3 translation(5, 3, -2);
    const math::Vec3 scale_factors(2, 0.5, 3);
    const math::Float rotation_angle = math::deg2rad(45.0);
    ...
    const math::Pt3 original(1, 0, 0);

    // Apply transformations step by step
    math::Pt3 after_rotate = R.transform_point(original);
    math::Pt3 after_scale = S.transform_point(after_rotate);
    math::Pt3 after_translate = T.transform_point(after_scale);

    // Should be same as combined transform
    math::Pt3 combined_result = combined.transform_point(original);
...
TEST_F(TransformTest, InverseTransform) {
    const math::Vec3 translation(3, -1, 5);
    Transform<double> T = Transform<double>::translate(translation);
    ...
    const math::Pt3 test_point(2, 4, -1);
    math::Pt3 transformed = T.transform_point(test_point);
    math::Pt3 back_to_original = T_inv.transform_point(transformed);
...
TEST_F(TransformTest, HasScale) {
    ...
    Transform<double> non_uniform_scale = Transform<double>::scale(math::Vec3(1.5, 2.0, 0.8));
    ...
    Transform<double> translation = Transform<double>::translate(math::Vec3(1, 2, 3));
    EXPECT_FALSE(translation.has_scale());

    // Rotation should have no scale
    Transform<double> rotation = Transform<double>::rotate_x(math::deg2rad(30.0));
...
TEST_F(TransformTest, NormalTransform) {
    ...
    const math::Vec3 scale_factors(2.0, 0.5, 1.0);
    Transform<double> S = Transform<double>::scale(scale_factors);

    // Create a normal vector pointing up
    math::Normal3 normal = math::Normal3::from_vector(math::Vec3(0, 1, 0));

    // Transform the normal
    math::Normal3 transformed_normal = S.transform_normal(normal);
...
TEST_F(TransformTest, RayTransform) {
    const math::Vec3 translation(1, 2, 3);
    Transform<double> T = Transform<double>::translate(translation);

    // Create a ray from origin pointing in +X direction
    Ray3 ray(math::Pt3(0, 0, 0), math::Vec3(1, 0, 0));
    ...
    // Test with rotation
    Transform<double> R = Transform<double>::rotate_z(math::deg2rad(90.0));
...
TEST_F(TransformTest, BoundsTransform) {
    ...
    math::Point<double, 3> min_pt(0, 0, 0);
    math::Point<double, 3> max_pt(1, 1, 1);
    ...
    const math::Vec3 translation(1, 1, 1);
    Transform<double> T = Transform<double>::translate(translation);
...
TEST_F(TransformTest, MatrixAccess) {
    ...
    const math::Mat4& matrix = T.matrix();
    ...
    const math::Mat4& inv_matrix = T.inverse_matrix();
...
TEST_F(TransformTest, SetMatrix) {
    ...
    math::Mat4 custom_matrix(1.0, 0.0, 0.0, 5.0, 0.0, 1.0, 0.0, 6.0, 0.0, 0.0, 1.0, 7.0, 0.0, 0.0, 0.0, 1.0);

    T.set_matrix(custom_matrix);

    // Test that the transform works correctly
    const math::Pt3 test_point(1, 1, 1);
    const math::Pt3 transformed = T.transform_point(test_point);
...
TEST_F(TransformTest, UniformScale) {
    const math::Float scale_factor = 3.0;
    Transform<double> S = Transform<double>::scale(scale_factor);

    const math::Pt3 test_point(1, 2, 3);
    const math::Pt3 transformed = S.transform_point(test_point);
...
TEST_F(TransformTest, AxisAngleRotationDifferentAxes) {
    const math::Float angle = static_cast<math::Float>(M_PI / 4.0);  // 45 degrees

    // Test rotation around X axis
    const math::Vec3 x_axis(1, 0, 0);
    ...
    const math::Pt3 test_point(0, 1, 0);
    math::Pt3 result_axis = Rx_axis.transform_point(test_point);
    math::Pt3 result_func = Rx_func.transform_point(test_point);
    ...
    const math::Pt3 test_point2(1, 0, 0);
    math::Pt3 result_axis2 = Ry_axis.transform_point(test_point2);
    math::Pt3 result_func2 = Ry_func.transform_point(test_point2);
...
TEST_F(TransformTest, LookAtAdvanced) {
    ...
    const math::Pt3 eye1(5, 5, 5);
    const math::Pt3 target1(0, 0, 0);
    const math::Vec3 up1(0, 1, 0);
    ...
    const math::Pt3 target_in_view1 = view1.transform_point(target1);
    EXPECT_LT(target_in_view1.z(), 0.0);

    // The eye position should be at origin in view space
    const math::Pt3 eye_in_view1 = view1.transform_point(eye1);
    ...
    const math::Pt3 eye2(0, 0, 5);
    const math::Pt3 target2(0, 0, 0);
    const math::Vec3 up2(0, 1, 0);

    Transform<double> view2 = Transform<double>::look_at(eye2, target2, up2);

    // Point to the right of camera (positive X in world)
    const math::Pt3 right_point(1, 0, 0);
    const math::Pt3 right_in_view = view2.transform_point(right_point);
    ...
    const math::Pt3 up_point(0, 1, 0);
    const math::Pt3 up_in_view = view2.transform_point(up_point);
    ...
    const math::Vec3 forward_world(0, 0, -1);
    const math::Vec3 forward_view = view2.transform_vector(forward_world);
    ...
    const math::Vec3 right_dir(1, 0, 0);
    const math::Vec3 up_dir(0, 1, 0);
    const math::Vec3 forward_dir(0, 0, -1);

    const math::Vec3 right_transformed = view2.transform_vector(right_dir);
    const math::Vec3 up_transformed = view2.transform_vector(up_dir);
    const math::Vec3 forward_transformed = view2.transform_vector(forward_dir);
...
TEST_F(TransformTest, Transpose) {
    ...
    const math::Mat4& orig_matrix = T.matrix();
    const math::Mat4& trans_matrix = T_transposed.matrix();
...
TEST_F(TransformTest, MixedTypes) {
    ...
    Transform<double> T = Transform<double>::translate(math::Vec3{1.0, 2.0, 3.0});

    // Test point transformation with mixed types
    math::Point<float, 3> pf{1.0f, 1.0f, 1.0f};
    auto result = T.transform_point(pf);  // Should create math::Point<double, 3>
    ...
    math::Vector<float, 3> vf{1.0f, 0.0f, 0.0f};
    auto vec_result = T.transform_vector(vf);  // Should create math::Vector<double, 3>
