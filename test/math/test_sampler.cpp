...
protected:
    void SetUp() override {
        mc_integrator = std::make_shared<math::MonteCarloIntegrator1D>();
        sampler_evaluator = std::make_shared<math::SamplerEvaluator<1>>();
        uniform_sampler = std::make_shared<math::UniformSampler<1>>();
    }

    std::shared_ptr<math::MonteCarloIntegrator1D> mc_integrator;
    std::shared_ptr<math::SamplerEvaluator<1>> sampler_evaluator;
    std::shared_ptr<math::UniformSampler<1>> uniform_sampler;
};
...
TEST_F(SamplerTest, SamplerEvaluator_UniformSampler) {
    ...
    auto uni_result = sampler_evaluator->evaluate(
        uniform_sampler->generate(sample_count)
    );
...
TEST_F(SamplerTest, SamplerEvaluator_StratifiedSampler) {
    int sample_count = 100;
    int strat_layer = 5;
    auto strat_result = sampler_evaluator->evaluate(
        std::make_shared<math::StratifiedSampler<1>>(strat_layer)->generate(sample_count)
    );
...
TEST_F(SamplerTest, Compare_Uniform_vs_Stratified_Variance) {
    ...
    // 测试UniformSampler
    auto uniform_result = sampler_evaluator->evaluate(
        uniform_sampler->generate(sample_count)
    );
    
    // 测试StratifiedSampler
    auto stratified_result = sampler_evaluator->evaluate(
        std::make_shared<math::StratifiedSampler<1>>(10)->generate(sample_count)
    );
...
TEST_F(SamplerTest, StratifiedSampler_DifferentLayers) {
    ...
    for (int layers : layer_counts) {
        auto stratified_sampler = std::make_shared<math::StratifiedSampler<1>>(layers);
...
TEST_F(SamplerTest, Point_Mid_Correctness) {
    // 创建已知的点集
    std::vector<math::Pt1> known_points = {
        math::Pt1{0.0f},
        math::Pt1{0.5f},
        math::Pt1{1.0f}
    };
    
    auto mid_point = math::Pt1::mid(known_points);
...
TEST_F(SamplerTest, Point_Mid_EdgeCases) {
    // 测试单个点
    std::vector<math::Pt1> single_point = {math::Pt1{0.7f}};
    auto mid_single = math::Pt1::mid(single_point);
    EXPECT_FLOAT_EQ(mid_single.x(), 0.7f);
    
    // 测试两个相同的点
    std::vector<math::Pt1> same_points = {math::Pt1{0.3f}, math::Pt1{0.3f}};
    auto mid_same = math::Pt1::mid(same_points);
...
TEST_F(SamplerTest, FullWorkflow_Performance) {
    ...
    auto uni_result = sampler_evaluator->evaluate(
        uniform_sampler->generate(100)
    );
    
    auto strat_result = sampler_evaluator->evaluate(
        std::make_shared<math::StratifiedSampler<1>>(5)->generate(100)
    );
