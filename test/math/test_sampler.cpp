...
TEST_F(SamplerTest, Compare_Uniform_vs_Stratified_Variance) {
    ...
    auto stratified_result =
        sampler_evaluator->evaluate(std::make_shared<math::StratifiedSampler<1>>(10)->generate(sample_count));
...
TEST_F(SamplerTest, Point_Mid_Correctness) {
    // 创建已知的点集
    std::vector<math::Pt1> known_points = {math::Pt1{0.0f}, math::Pt1{0.5f}, math::Pt1{1.0f}};
...
TEST_F(SamplerTest, FullWorkflow_Performance) {
    ...
    auto strat_result = sampler_evaluator->evaluate(std::make_shared<math::StratifiedSampler<1>>(5)->generate(100));
