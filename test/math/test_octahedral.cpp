...
TEST_F(OctahedralVectorTest, BasicEncodeDecode) {
    std::vector<Vector<Float, 3>> test_vectors = {
        unit_x, unit_y, unit_z, unit_neg_x, unit_neg_y, unit_neg_z, diagonal_positive, diagonal_negative};
    ...
    for (const auto& original : test_vectors) {
        ...
        EXPECT_TRUE(reconstructed.is_normalized()) << "Reconstructed vector should be normalized for input";
...
TEST_F(OctahedralVectorTest, BasicProperties) {
    std::vector<Vector<Float, 3>> test_vectors = {unit_x, unit_y, unit_z, diagonal_positive};
