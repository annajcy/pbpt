...
TEST_F(TransformTest, LookAt) {
    ...
    // In view space, X should be 5, Y should be 0, Z should be -10 (10 units away from camera looking down -Z)
...
TEST_F(TransformTest, Orthographic) {
    ...
    EXPECT_NEAR(p1_ndc.z(), 1.0, kEpsilon); // Far plane maps to 1 with this formula
    ...
    EXPECT_NEAR(p2_ndc.z(), 0.0, kEpsilon);  // Near plane maps to 0 with this formula
}
