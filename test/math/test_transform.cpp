...
TEST_F(TransformTest, Translate) {
    ...
    const Mat4 t = Transform<double>::translate(translation_vec).mat();
...
TEST_F(TransformTest, Scale) {
    ...
    const Mat4 s = Transform<double>::scale(scale_vec).mat();
...
TEST_F(TransformTest, RotateZ90Degrees) {
    const Mat4 rz            = Transform<double>::rotate_z(deg2rad(90.0)).mat();
...
TEST_F(TransformTest, RotateY90Degrees) {
    const Mat4 ry = Transform<double>::rotate_y(deg2rad(90.0)).mat();
...
TEST_F(TransformTest, RotateX90Degrees) {
    const Mat4 rx = Transform<double>::rotate_x(deg2rad(90.0)).mat();
...
TEST_F(TransformTest, RotateAxisAngle) {
    ...
    // version
    const Mat4 r_axis_angle = Transform<double>::rotate(angle, z_axis).mat();
    const Mat4 r_z          = Transform<double>::rotate_z(angle).mat();
...
TEST_F(TransformTest, LookAt) {
    ...
    const Vec3 up(0, 1, 0);
    const Mat4 view_matrix = Transform<double>::look_at(eye, target, up).mat();
...
TEST_F(TransformTest, Orthographic) {
    const Mat4 ortho_matrix = Transform<double>::orthographic(-10.0, 10.0, -5.0, 5.0, 1.0, 101.0).mat();
...
TEST_F(TransformTest, Perspective) {
    ...
    const Mat4  persp_matrix = Transform<double>::perspective(fov_rad, aspect, z_near, z_far).mat();
