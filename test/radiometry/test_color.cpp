...
protected:
    void SetUp() override {
        ...
        srgb_color_space = std::make_unique<RGBColorSpace<double>>(
            Point<double, 2>(0.64, 0.33),    // Red primary
...
TEST_F(ColorTest, sRGBColorSpaceAdvancedOperations) {
    ...
    std::vector<ColorTestCase> test_cases = {
        {RGB<double>(1.0, 1.0, 1.0), "white", LAB<double>(100, 0, 0)},
...
TEST_F(ColorTest, RGBAlbedoSpectrumIntegration) {
    ...
    RGBAlbedoSpectrumDistribution<double, RGBSigmoidPolynomialNormalized> albedo({
