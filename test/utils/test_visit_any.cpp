...
TEST(VisitAnyTest, VisitOneModification) {
    ...
    visit_one([](auto& arg) { arg += 5; }, v);

    EXPECT_EQ(std::get<int>(v), 15);

    int direct = 20;
    visit_one([](auto& arg) { arg += 5; }, direct);
...
TEST(VisitAnyTest, VisitAnyAllVariant) {
    ...
    auto result = visit_any([](auto a, auto b) { return static_cast<double>(a) + static_cast<double>(b); }, v1, v2);
...
TEST(VisitAnyTest, VisitAnyAllDirect) {
    ...
    auto result = visit_any([](auto x, auto y) { return x + y; }, a, b);
