...
int main() {
    ...
    std::array<geometry::Ray3, 2> rds = {geometry::Ray3(math::Pt3{0.1, 0.0, 0.0}, math::Vec3{1.0, 0.1, 0.0}),
                                         geometry::Ray3(math::Pt3{0.0, 0.1, 0.0}, math::Vec3{1.0, 0.0, 0.1})};

    geometry::RayDiff3 ray_diff(ray, rds);
    ray_diff.scale(2.0);
    std::cout << "Ray Differential Origin: " << ray_diff.main_ray().origin()
              << ", Direction: " << ray_diff.main_ray().direction() << std::endl;
    ...
    geometry::DirectionalCone<math::Float> dc(math::Vector<math::Float, 3>(0, 0, 1), math::deg2rad(80.0));
    ...
    auto res_para = integrator::integrate<double>(
        para,
        [&shading_p, &shading_p_normal](const integrator::SurfaceInfo<double>& surface) {
    ...
    auto sampled_spectrum = black_body_spectrum.sample<4>(
        radiometry::SampledSpectrum<double, 4>(math::Vector<double, 4>(400.0, 600.0, 800.0, 1000.0)));
    ...
    auto xyz_ = pbpt::radiometry::make_spectra_from_csv<double, radiometry::constant::XYZRange>(
        (cur_path / "CIE_xyz_1931_2deg.csv").string(), 3);
    auto Xbar = xyz_[0];
    ;
    auto Ybar = xyz_[1];
    ;
    ...
    auto pppp =
        Ybar.sample<5>(radiometry::SampledWavelength<double, 5>(math::Vector<double, 5>(400, 500, 600, 700, 800)));
    std::cout << pppp << std::endl;

    auto D50 = pbpt::radiometry::make_spectra_from_csv<double, radiometry::constant::luminantD50Range>(
        (cur_path / "CIE_std_illum_D50.csv").string(), 1)[0];
    auto D65 = pbpt::radiometry::make_spectra_from_csv<double, radiometry::constant::luminantD65Range>(
        (cur_path / "CIE_std_illum_D65.csv").string(), 1)[0];
    auto A = pbpt::radiometry::make_spectra_from_csv<double, radiometry::constant::luminantARange>(
        (cur_path / "CIE_std_illum_A.csv").string(), 1)[0];
    ...
    auto d50_sampled =
        D50.sample<5>(radiometry::SampledWavelength<double, 5>(math::Vector<double, 5>(400, 560, 600, 700, 800)));
    auto d65_sampled =
        D65.sample<5>(radiometry::SampledWavelength<double, 5>(math::Vector<double, 5>(400, 560, 600, 700, 800)));
    auto a_sampled =
        A.sample<5>(radiometry::SampledWavelength<double, 5>(math::Vector<double, 5>(400, 560, 600, 700, 800)));
    auto black_body_sampled = black_body_spectrum.sample<5>(
        radiometry::SampledWavelength<double, 5>(math::Vector<double, 5>(400, 560, 600, 700, 800)));
    ...
    for (int i = 0; i < round_N; i++) {
        ...
        auto wl_r =
            rng.generate_uniform(radiometry::constant::lambda_min<double>, radiometry::constant::lambda_max<double>);
        ...
        auto xyz = radiometry::XYZ<double>::from_sampled_spectrum(
            D65.sample(wl), wl,
            radiometry::SampledPdf<double, sample_N>(math::Vector<double, sample_N>::filled(
                1 / (radiometry::constant::lambda_max<double> - radiometry::constant::lambda_min<double>))));
    ...
    std::cout << "sRGB White Point to LAB: " << radiometry::LAB<double>::from_xyz(white_xyz, sRGB.white_point())
              << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "Albedo Coefficients (C0, C1, C2): " << albedo_unnormalized.c0 << ", " << albedo_unnormalized.c1
              << ", " << albedo_unnormalized.c2 << std::endl;
    ...
    auto [error, coeffs] = radiometry::optimize_albedo_rgb_sigmoid_polynomial(
        radiometry::RGB<double>(0.139, 0.735, 0.989), radiometry::constant::sRGB<double>, D65);
    std::cout << "Optimization Error: " << error << std::endl;
    std::cout << "Optimized Albedo Coefficients (C0, C1, C2): " << coeffs[0] << ", " << coeffs[1] << ", " << coeffs[2]
              << std::endl;

    auto optimized_sigmoid_polynomial_unnormalized =
        radiometry::RGBSigmoidPolynomialNormalized<double>{coeffs[0], coeffs[1], coeffs[2]}.to_unnormalized();
    std::cout << "Optimized Albedo Coefficients Unnormalized (C0, C1, C2): "
              << optimized_sigmoid_polynomial_unnormalized.c0 << ", " << optimized_sigmoid_polynomial_unnormalized.c1
              << ", " << optimized_sigmoid_polynomial_unnormalized.c2 << std::endl;

    radiometry::RGBAlbedoSpectrumDistribution<double, radiometry::RGBSigmoidPolynomial> optimized_albedo(
        optimized_sigmoid_polynomial_unnormalized);
    ...
    auto rspn = radiometry::RGBSigmoidPolynomialNormalized<double>{opti.normalized_coeffs[0], opti.normalized_coeffs[1],
                                                                   opti.normalized_coeffs[2]};
    radiometry::RGBUnboundedSpectrumDistribution<double, radiometry::RGBSigmoidPolynomialNormalized>
        unbounded_rgb_spectrum(rspn, scale);
    ...
    radiometry::RGBIlluminantSpectrumDistribution<
        double, radiometry::RGBSigmoidPolynomialNormalized,
    ...
    // Build a simple pixel sensor + film for the demo camera
    auto ps = camera::PixelSensor<double, radiometry::constant::CIED65SpectrumType<double>,
                                  radiometry::constant::CIED65SpectrumType<double>,
                                  radiometry::constant::XYZSpectrumType<double>>(
        radiometry::constant::CIE_D65_ilum<double>, radiometry::constant::CIE_D65_ilum<double>,
        radiometry::constant::sRGB<double>,
        radiometry::ResponseSpectrum<radiometry::constant::XYZSpectrumType<double>>(
            radiometry::constant::CIE_X<double>, radiometry::constant::CIE_Y<double>,
            radiometry::constant::CIE_Z<double>),
        1.0);
    auto demo_film = camera::HDRFilm<double, decltype(ps)>(math::Vector<int, 2>(1000, 1000), ps);

    camera::ThinLensPerspectiveCamera<double> camera(camera::AnyFilm<double>(std::move(demo_film)),
                                                     45.0,  // fov_degrees
                                                     camera::FovAxis::Smaller, -0.1, -100.0,
                                                     1.0  // near, far, focal_distance
    );

    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 10; j++) {
            auto r_ = camera.generate_ray(camera::CameraSample<double>{
                math::Point<double, 2>(i * 100 + 50, j * 100 + 50), math::Point<double, 2>(0.3, 0.3)});

            std::cout << "Center Camera Ray Origin: " << r_.origin() << ", Direction: " << r_.direction() << std::endl;
        }
    }

    radiometry::XYZ<double> d65_xyz =
        radiometry::XYZ<double>::from_illuminant(radiometry::constant::CIE_D65_ilum<double>);
    ...
    radiometry::XYZ<double> xyz_red =
        radiometry::XYZ<double>::from_reflectance(ref_red, radiometry::constant::CIE_D65_ilum<double>);
    ...
    radiometry::XYZ<double> xyz_blue =
        radiometry::XYZ<double>::from_reflectance(ref_blue, radiometry::constant::CIE_D65_ilum<double>);
    ...
    auto ref_blue_ =
        radiometry::constant::get_swatch_reflectance<double>(radiometry::constant::SwatchReflectance::BlueSky);
    radiometry::XYZ<double> xyz_blue_ =
        radiometry::XYZ<double>::from_reflectance(ref_blue_, radiometry::constant::CIE_D65_ilum<double>);
    ...
    auto white_ref =
        radiometry::constant::get_swatch_reflectance<double>(radiometry::constant::SwatchReflectance::White);
    radiometry::XYZ<double> xyz_white =
        radiometry::XYZ<double>::from_reflectance(white_ref, radiometry::constant::CIE_D65_ilum<double>);
    ...
    auto black_ref =
        radiometry::constant::get_swatch_reflectance<double>(radiometry::constant::SwatchReflectance::Black);
    radiometry::XYZ<double> xyz_black =
        radiometry::XYZ<double>::from_reflectance(black_ref, radiometry::constant::CIE_D65_ilum<double>);
