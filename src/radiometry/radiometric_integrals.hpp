...
/// Sample paired with its PDF value.
 ...
 *
 * A domain defines:
 * - @c sample_one_impl() to draw a sample and return (sample, pdf).
 * - @c pdf_impl() to return a constant PDF in simple cases.
 *
...
/// Directional sample type: 3D direction and associated PDF.
template<typename T>
using DirectionSample = Sample<math::Vector<T, 3>, T>;

/**
 * @brief Uniform sampling of the unit hemisphere with respect to solid angle.
 */
template<typename T>
...
public:
    /// Type of samples generated by this domain (unit directions).
...
/**
 * @brief Cosine-weighted (projected) hemisphere sampling domain.
 *
 * Samples directions on the hemisphere with a PDF proportional to
 * cos(theta), which is useful for Lambertian BRDF integration.
 */
template<typename T>
...
/// Disk sample type: 2D point and associated PDF.
template<typename T>
...
public:
    /// Type of samples generated by this domain (2D disk points).
...
/// Surface sample consisting of a SurfaceInfo and its PDF.
template<typename T>
