...
public:
    ...
    std::vector<std::pair<int, std::vector<T>>> load_data() {
        ...
        if (!fin)
            throw std::runtime_error("Cannot open CSV: " + file_path_);
...
private:
    bool should_skip_line(const std::string& line) const {
        return line.empty() || line[0] == '#' || line.find_first_of("0123456789") == std::string::npos;
    }
...
inline auto make_spectra_from_csv(const std::string& csv_path, int spectrum_count) {
    ...
    for (int i = 0; i < spectrum_count; ++i) {
        result.push_back(
            radiometry::TabularSpectrumDistribution<T, Range::LMinValue, Range::LMaxValue>(spectra_data[i]));
    }
    return result;
}
