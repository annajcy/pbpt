...
class Light {
public:
    geometry::Transform<T> render_to_light_transform() const { return as_derived().render_to_light_transform_impl(); }

    geometry::Transform<T> light_to_render_transform() const { return as_derived().light_to_render_transform_impl(); }
    ...
    std::optional<LightSampleResult<T, N, NormalInteractionType>> sample_light(
        ...
        return as_derived().sample_light_impl(sampled_wavelengths, ref_point, u_sample);
    }
    ...
    T sample_light_pdf(const NormalInteractionType& ref_point, const math::Vector<T, 3>& wi) const {
        return as_derived().sample_light_pdf_impl(ref_point, wi);
    }
