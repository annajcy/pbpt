...
struct LdsSamplerSerde {
    ...
    static constexpr std::string_view xml_type = "independent";
    ...
    static load_result load(const pugi::xml_node& node, LoadContext<T>& ctx) {
        ...
        for (auto child : node.children("integer")) {
            if (std::string(child.attribute("name").value()) == "sample_count") {
                sample_count = child.attribute("value").as_int(4);
            }
        }
        if (sample_count <= 0) {
            throw std::runtime_error("invalid sample_count for independent sampler: must be > 0");
        }
    ...
    static void write(const write_target& target, pugi::xml_node& node, WriteContext<T>& ctx) {
        ...
        sample_count_node.append_attribute("name") = "sample_count";
