...
void write_texture_nodes(pugi::xml_node& root, WriteContext<T>& ctx) {
    for (const auto& [texture_name, texture_id] : ctx.scene.resources.reflectance_texture_library.name_to_id()) {
        const auto& any_texture = ctx.scene.resources.reflectance_texture_library.get(texture_id);
...
void write_bsdf_nodes(pugi::xml_node& root, WriteContext<T>& ctx) {
    for (const auto& [material_name, material_id] : ctx.scene.resources.any_material_library.name_to_id()) {
        const auto& any_material = ctx.scene.resources.any_material_library.get(material_id);
...
void write_shape_nodes(pugi::xml_node& root, WriteContext<T>& ctx) {
    if (ctx.scene.resources.shape_instances.empty() && !ctx.scene.resources.mesh_library.name_to_id().empty()) {
        for (const auto& [mesh_name, mesh_id] : ctx.scene.resources.mesh_library.name_to_id()) {
            const auto& mesh = ctx.scene.resources.mesh_library.get(mesh_id);
            ...
            if (!ctx.scene.resources.mesh_material_map.contains(mesh_name)) {
                throw std::runtime_error("Mesh has no material assignment: " + mesh_name);
            }
            int material_id = ctx.scene.resources.mesh_material_map.at(mesh_name);
            ...
            for (const auto& [light_key, light_id] : ctx.scene.resources.mesh_light_map) {
        ...
        for (const auto& record : ctx.scene.resources.shape_instances) {
            auto shape = root.append_child("shape");
            dispatch_write_shape<T, ShapeSerdeList<T>>(record, shape, ctx);
        }
    }
...
void write_scene(const scene::Scene<T>& scene, const std::string& filename) {
    ...
    WriteContext<T> ctx{scene, scene_dir, mesh_dir, texture_dir};
    ...
    dispatch_write_integrator<T, IntegratorSerdeList<T>>(scene.serialization_meta.integrator_type, scene, integrator,
                                                         ctx);
    ...
    dispatch_write_sampler<T, SamplerSerdeList<T>>(scene.serialization_meta.sampler_type, scene, sensor, ctx);
