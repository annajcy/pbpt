...
texture::Image<math::Vector<T, 3>> read_hdr_image(const std::filesystem::path& input_path) {
    ...
    try {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                int flipped_y = height - 1 - y;  // Flip Y axis for EXR coordinate system
                std::size_t src_idx = (static_cast<std::size_t>(flipped_y) * static_cast<std::size_t>(width) +
...
void write_ldr_image(const std::filesystem::path& output_path, const texture::Image<math::Vector<T, 3>>& image) {
    ...
    std::vector<unsigned char> data(static_cast<std::size_t>(width) * static_cast<std::size_t>(height) * 3);
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            auto clamp = [](T val) { return std::max(T(0), std::min(T(1), val)); };
...
texture::Image<math::Vector<T, 3>> read_ldr_image(const std::filesystem::path& input_path) {
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            image.get_pixel(x, y) =
                math::Vector<T, 3>(static_cast<T>(data[idx + 0]) * scale, static_cast<T>(data[idx + 1]) * scale,
