...
private:
    ...
    static void intersectFunc(const RTCIntersectFunctionNArguments* args) {
        if (args->N != 1)
            return;  // Only support single ray for now
        ...
        geometry::Ray<T, 3> geo_ray(original_ray->origin(), original_ray->direction(), static_cast<T>(ray.tfar),
                                    original_ray->t_min());
...
public:
    ...
    EmbreeAggregate(std::vector<shape::Primitive<T>> primitives) : m_primitives(std::move(primitives)) {
        m_device = rtcNewDevice(nullptr);
