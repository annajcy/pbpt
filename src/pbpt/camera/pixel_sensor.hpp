 ...
 * spectral radiance and that channel's sensitivity
...
public:
     ...
     * wavelength of:
     * radiance(lambda) * sensor_response(lambda).
    ...
    radiometry::RGB<T> radiance_to_sensor_rgb(const SpectrumType& radiance) const {
        auto sensor_rgb =
            radiometry::project_emission<T, radiometry::RGB, SpectrumType, SensorResponseSpectrumType>(
                radiance, m_sensor_response);
     ...
     *   radiance(lambda_i) * sensor_response(lambda_i) / p(lambda_i)
    ...
    radiometry::RGB<T> radiance_to_sensor_rgb(const radiometry::SampledSpectrum<T, N>& radiance,
        ...
        auto sensor_rgb =
            radiometry::project_sampled_emission<T, radiometry::RGB, N, SensorResponseSpectrumType>(
                radiance, wavelengths, pdf, m_sensor_response);
        return sensor_rgb * m_image_ratio;
    }
