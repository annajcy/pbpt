...
public:
     ...
     */
    math::Vector<int, 2> film_resolution() const { return as_derived().film_resolution_impl(); }
...
public:
     ...
     */
    CameraProjection(const geometry::Transform<T>& camera_to_clip, const geometry::Transform<T>& clip_to_viewport)
    ...
    geometry::Transform<T> camera_to_clip() const { return m_camera_to_clip; }

    /// Get the clip-to-viewport transform.
    geometry::Transform<T> clip_to_viewport() const { return m_clip_to_viewport; }

    /// Get the camera-to-viewport transform.
    geometry::Transform<T> camera_to_viewport() const { return m_camera_to_viewport; }

    /// Get the inverse transform: viewport-to-clip.
    geometry::Transform<T> viewport_to_clip() const { return m_clip_to_viewport.inversed(); }

    /// Get the inverse transform: clip-to-camera.
    geometry::Transform<T> clip_to_camera() const { return m_camera_to_clip.inversed(); }

    /// Get the inverse transform: viewport-to-camera.
    geometry::Transform<T> viewport_to_camera() const { return m_camera_to_viewport.inversed(); }
     ...
     */
    static math::Vector<T, 2> calculate_physical_film_size(T fov_degrees, const std::string& fov_axis, T near_clip,
    ...
    static CameraProjection<T> create_orthographic_projection(const math::Vector<int, 2>& film_resolution,
                                                              const math::Vector<T, 2>& film_physical_size, T near,
                                                              T far) {
        return CameraProjection<T>::orthographic(-film_physical_size.x() / 2, film_physical_size.x() / 2,
                                                 -film_physical_size.y() / 2, film_physical_size.y() / 2, near, far,
     ...
     */
    static CameraProjection<T> create_orthographic_projection_by_fov(const math::Vector<int, 2>& film_resolution, T fov,
                                                                     const std::string& fov_axis, T near, T far) {
        auto film_size = calculate_physical_film_size(fov, fov_axis, near, film_resolution.x(), film_resolution.y());
        return create_orthographic_projection(film_resolution, film_size, near, far);
    }
     ...
     */
    static CameraProjection<T> create_perspective_projection_by_fov(const math::Vector<int, 2>& film_resolution,
                                                                    T fov_degrees, const std::string& fov_axis, T near,
                                                                    T far) {
        auto film_size =
            calculate_physical_film_size(fov_degrees, fov_axis, near, film_resolution.x(), film_resolution.y());
        return create_perspective_projection(film_resolution, film_size, near, far);
    }
};
