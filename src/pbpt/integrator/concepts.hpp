...
concept RenderLoopContextConcept = requires(const SceneContextT& context) {
    context.camera;
    context.film;
    ...
    camera::PixelFilterSampleConcept<std::remove_cvref_t<decltype(std::declval<const SceneContextT&>().pixel_filter)>, T> &&
    camera::FilmAccumulationConcept<std::remove_cvref_t<decltype(std::declval<SceneContextT&>().film)>, T, N> &&
    camera::RenderTransformRayConcept<std::remove_cvref_t<decltype(std::declval<const SceneContextT&>().render_transform)>, T> &&
...
concept PathTraceContextConcept =
    RenderLoopContextConcept<SceneContextT, T, N> &&
    requires(const SceneContextT& context) {
        context.aggregate;
    } && aggregate::AggregateIntersectConcept<
            std::remove_cvref_t<decltype(std::declval<const SceneContextT&>().aggregate)>, T>;
