...
protected:
    ...
    void render_loop(const SceneContextT& context, int spp, const std::string& output_path,
        ...
        const std::size_t total_pixels =
            static_cast<std::size_t>(resolution.x()) * static_cast<std::size_t>(resolution.y());
        ...
        for (int y = 0; y < resolution.y(); ++y) {
            ...
            for (int x = 0; x < resolution.x(); ++x) {
                ...
                for (int s = 0; s < spp; ++s) {
                    ...
                    auto sample =
                        camera::CameraSample<T>::create_thinlens_sample(filtered_sample.film_position, lens_position);
                    ...
                    context.film.template add_sample<N>(pixel, Li, wavelength_sample, wavelength_pdf,
                                                        filtered_sample.weight);
                }
            }
