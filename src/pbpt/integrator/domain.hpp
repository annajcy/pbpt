 ...
 *
 * A domain defines:
 * - @c sample_impl() to draw a sample and return (sample).
...
public:
    ...
    auto sample(RNG& rng2d) const {
        return as_derived().sample_impl(rng2d);
    }

    /// Return the PDF of this domain.
    constexpr auto pdf(const SampleType& sample) const { return as_derived().pdf_impl(sample); }
...
template <typename T>
class UniformHemisphereDomain : public Domain<T, UniformHemisphereDomain<T>, math::Vector<T, 3>> {
    friend class Domain<T, UniformHemisphereDomain<T>, math::Vector<T, 3>>;
...
private:
    ...
    constexpr T pdf_impl(const math::Vector<T, 3>& sample) const { return sampler::sample_uniform_hemisphere_pdf<T>(); }
};
 ...
 */
template <typename T>
class CosineWeightedHemisphereDomain : public Domain<T, CosineWeightedHemisphereDomain<T>, math::Vector<T, 3>> {
    friend class Domain<T, CosineWeightedHemisphereDomain<T>, math::Vector<T, 3>>;
 ...
 */
template <typename T>
class UniformDiskDomain : public Domain<T, UniformDiskDomain<T>, math::Point<T, 2>> {
    friend class Domain<T, UniformDiskDomain<T>, math::Point<T, 2>>;
...
private:
    ...
    constexpr T pdf_impl(const math::Point<T, 2>& /*sample*/
    ) const {
 ...
 */
template <typename T>
class UniformParallelogramAreaDomain : public Domain<T, UniformParallelogramAreaDomain<T>, SurfaceInfo<T>> {
    friend class Domain<T, UniformParallelogramAreaDomain<T>, SurfaceInfo<T>>;
...
public:
     ...
     * the edge vectors.
     */
    UniformParallelogramAreaDomain(const math::Point<T, 3>& origin, const math::Vector<T, 3>& edge1,
    ...
    constexpr T area() const { return m_area; }

    /// Returns the unit surface normal of the parallelogram.
    constexpr math::Normal<T, 3> normal() const { return m_normal; }

private:
    template <typename RNG>
    SurfaceInfo<T> sample_impl(RNG& rng2d) const {
    ...
    constexpr T pdf_impl(const SurfaceInfo<T>& /*sample*/
    ) const {
        return T(1) / area();
    }
};
...
auto integrate(const Domain& domain, const Func& func, int sample_count, RNG& rng) {
    using R = decltype(std::declval<const Func&>()(std::declval<typename Domain::sample_type>()) /
                       static_cast<T>(std::declval<int>()));

    R result = R{};
    domain.foreach_sample(sample_count, rng,
                          [&](const auto& sample, const auto& pdf) { result += func(sample) / pdf; });
