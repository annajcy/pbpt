...
inline T fresnel_dielectric(T cos_theta_i, T eta) {
    ...
    T r_perp = (cos_theta_i - (eta * cos_theta_t)) / (cos_theta_i + (eta * cos_theta_t));
...
inline T fresnel_conductor_per_wavelength(T cos_theta_i, const math::Complex<T>& eta) {
    ...
    math::Complex<T> r_parl = (eta * cos_theta_i - cos_theta_t) / (eta * cos_theta_i + cos_theta_t);
    math::Complex<T> r_perp = (cos_theta_i - eta * cos_theta_t) / (cos_theta_i + eta * cos_theta_t);
    return (r_perp.squared_length() + r_parl.squared_length()) / T(2);
}

template <typename T, int N>
radiometry::SampledSpectrum<T, N> fresnel_conductor(T cos_theta_i, const radiometry::SampledSpectrum<T, N>& eta,
...
template <typename T, int N>
radiometry::SampledSpectrum<T, N> fresnel_conductor_schlick(T cos_theta_i, T eta,
