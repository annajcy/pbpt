...
public:
    ...
    BSDF(const geometry::SurfaceInteraction<T>& si, const geometry::ShadingInfo<T>& shading, BxDFType&& bxdf)
        : m_frame(build_shading_frame(si, shading)), m_bxdf(AnyBxDF<T, N>(std::move(bxdf))) {}

    BSDF(const geometry::Frame<T>& shading_frame, AnyBxDF<T, N> bxdf)
        : m_frame(shading_frame), m_bxdf(std::move(bxdf)) {}

    math::Vector<T, 3> render_to_local(const math::Vector<T, 3>& w) const { return m_frame.to_local(w); }

    math::Vector<T, 3> local_to_render(const math::Vector<T, 3>& w) const { return m_frame.to_render(w); }

    // Evaluate f()
    radiometry::SampledSpectrum<T, N> f(const radiometry::SampledWavelength<T, N>& swl, const math::Vector<T, 3>& wo,
                                        const math::Vector<T, 3>& wi, TransportMode mode,
    ...
    // Evaluate PDF()
    T pdf(const math::Vector<T, 3>& wo, const math::Vector<T, 3>& wi, TransportMode mode,
          const BxDFReflTransFlags sample_flags = BxDFReflTransFlags::All) const {
        auto wo_local = m_frame.to_local(wo);
        auto wi_local = m_frame.to_local(wi);
        return std::visit([&](const auto& bxdf) { return bxdf.pdf(wo_local, wi_local, mode, sample_flags); }, m_bxdf);
    }

    // Sample BSDF
    std::optional<BxDFSampleRecord<T, N>> sample_f(
        const radiometry::SampledWavelength<T, N>& swl, const math::Vector<T, 3>& wo, const T uc,
        const math::Point<T, 2>& u2, TransportMode mode,
        const BxDFReflTransFlags sample_flags = BxDFReflTransFlags::All) const {
        auto wo_local = m_frame.to_local(wo);
        auto bxdf_sample = std::visit(
            [&](const auto& bxdf) { return bxdf.sample_f(swl, wo_local, uc, u2, mode, sample_flags); }, m_bxdf);
        if (!bxdf_sample)
            return std::nullopt;
