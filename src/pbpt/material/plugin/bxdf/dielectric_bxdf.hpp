...
private:
    using AnyDielectricBxDF = std::variant<DielectricSpecularBxDF<T, N>, DielectricRoughBxDF<T, N>, NullBxDF<T, N>>;

    AnyDielectricBxDF m_bxdf;

public:
    DielectricBxDF(T eta, const MicrofacetModel<T>& microfacet_model) : m_bxdf(select_bxdf(eta, microfacet_model)) {}
