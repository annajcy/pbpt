...
private:
    using AnyConductorBxDF = std::variant<ConductorSpecularBxDF<T, N>, ConductorRoughBxDF<T, N>>;

    AnyConductorBxDF m_bxdf;

public:
    ConductorBxDF(const radiometry::SampledSpectrum<T, N>& eta, const radiometry::SampledSpectrum<T, N>& k,
                  const MicrofacetModel<T>& microfacet_model)
        : m_bxdf(microfacet_model.is_delta() ? AnyConductorBxDF(ConductorSpecularBxDF<T, N>(eta, k))
