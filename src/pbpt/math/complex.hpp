...
public:
    static Complex<T> from_vector(const math::Vector<T, 2>& vec) { return Complex<T>(vec.x(), vec.y()); }
...
public:
    ...
    constexpr Complex<T> operator-() const noexcept { return Complex<T>(-m_real, -m_imag); }
    ...
    constexpr Complex<T> exp() const noexcept {
        ...
        return Complex<T>(exp_real * std::cos(m_imag), exp_real * std::sin(m_imag));
    }
