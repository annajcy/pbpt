...
public:
     ...
     */
    Sphere(const geometry::Transform<T>& object_to_render, bool reverse_orientation, T radius, T z_min, T z_max,
           T phi_max)
        : m_radius(radius),
     ...
     */
    Sphere(const geometry::Transform<T>& object_to_render, bool reverse_orientation, T radius)
    ...
    T z_min_theta() const { return std::acos(std::clamp(m_z_min / m_radius, T(-1), T(1))); }

    T z_max_theta() const { return std::acos(std::clamp(m_z_max / m_radius, T(-1), T(1))); }
...
private:
    ...
    geometry::Transform<T> object_to_render_transform_impl() const { return m_object_to_render; }

    geometry::Transform<T> render_to_object_transform_impl() const { return m_render_to_object; }

    geometry::Bounds<T, 3> bounding_box_impl() const {
        // 1. Create Object Space Bounds
        geometry::Bounds<T, 3> object_bounds(math::Point<T, 3>(-m_radius, -m_radius, m_z_min),
    ...
    geometry::DirectionalCone<T> normal_bounding_cone_impl() const {
        ...
        return geometry::DirectionalCone<T>(math::Vector<T, 3>(axis_render),  // 转换回 Vector 传递给构造函数
                                            cos_theta_obj);
    }
    ...
    std::optional<IntersectionRecord<T>> intersect_ray_differential_impl(
    ...
    ShapeSample<T> sample_on_shape_impl(const math::Point<T, 2>& u_sample) const {
        ...
        if (m_object_to_render.is_swaps_handedness())
            n_render = -n_render;
    ...
    ShapeSample<T> sample_on_solid_angle_impl(const math::Point<T, 3>& ref, const math::Point<T, 2>& u_sample) const {
        ...
        bool is_partial = (std::abs(m_phi_max - 2 * math::pi_v<T>) > 1e-4f) || (m_z_min > -m_radius + 1e-4f) ||
        ...
        math::Normal<T, 3> n_obj =
            math::Normal<T, 3>(sin_alpha * std::cos(phi) * w_x + sin_alpha * std::sin(phi) * w_y + cos_alpha * w_z)
                .normalized();
    ...
    T sample_on_solid_angle_pdf_impl(const math::Point<T, 3>& ref, const math::Point<T, 3>& p_surface) const {
...
private:
    ...
    std::optional<SphereIntersectionResult<T>> intersect_object_space(const geometry::Ray<T, 3>& ray) const {
        ...
        if (p_refined.x() == 0 && p_refined.y() == 0 && p_refined.z() == 0)
            p_refined = p_hit;  // Safety

        auto phi = std::atan2(p_refined.y(), p_refined.x());
        if (phi < 0)
            phi += 2 * math::pi_v<T>;

        if (p_refined.z() < m_z_min || p_refined.z() > m_z_max || phi > m_phi_max)
            return std::nullopt;

        return SphereIntersectionResult<T>{math::Point<T, 3>(p_refined), T(t_hit), T(phi)};
    }

    geometry::SurfaceInteraction<T> compute_object_interaction(const geometry::Ray<T, 3>& ray_obj,
                                                               const SphereIntersectionResult<T>& intersection) const {
        ...
        T z_radius = std::sqrt(std::max(
            T(0), intersection.p_obj.x() * intersection.p_obj.x() + intersection.p_obj.y() * intersection.p_obj.y()));
        ...
        math::Vector<T, 3> dpdu(-m_phi_max * intersection.p_obj.y(), m_phi_max * intersection.p_obj.x(), 0);

        // dp/dv: Tangent along longitude
        T theta_range = z_max_theta() - z_min_theta();
        T sin_theta = std::sqrt(std::max(T(0), T(1) - cos_theta * cos_theta));

        math::Vector<T, 3> dpdv(p_hit_z * cos_phi, p_hit_z * sin_phi, -m_radius * sin_theta);
        ...
        // 3. Compute Normal
        math::Normal<T, 3> N =
            math::Normal<T, 3>::from_vector((intersection.p_obj - math::Point<T, 3>(0, 0, 0)).normalized());

        // 4. Compute Error Bounds (for robust intersection)
        math::Vector<T, 3> p_error =
            math::gamma<T>(5) * math::Vector<T, 3>(std::abs(intersection.p_obj.x()), std::abs(intersection.p_obj.y()),
                                                   std::abs(intersection.p_obj.z()));

        return geometry::SurfaceInteraction<T>(intersection.p_obj - p_error, intersection.p_obj + p_error,
                                               -ray_obj.direction().normalized(), N, math::Point<T, 2>(u, v), dpdu,
                                               dpdv, dndu, dndv);
    }
};
