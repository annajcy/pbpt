...
template <typename T>
void register_texture_loaders(TextureLoaderRegistry<T>& registry) {
    registry.register_loader("bitmap", [](const pugi::xml_node& node, LoaderContext<T>& ctx) -> texture::AnyTexture<T> {
