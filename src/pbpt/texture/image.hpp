...
inline ImageN<T, 3> image4_to_image3(const ImageN<T, 4>& img4) {
    ...
    std::transform(img4.data().begin(), img4.data().end(), img3.data().begin(),
                   [](const math::Vector<T, 4>& v) { return math::Vector<T, 3>(v.r(), v.g(), v.b()); });
    return img3;
}
...
inline ImageN<T, 4> image3_to_image4(const ImageN<T, 3>& img3, T alpha = static_cast<T>(1)) {
    ...
    std::transform(img3.data().begin(), img3.data().end(), img4.data().begin(),
                   [alpha](const math::Vector<T, 3>& v) { return math::Vector<T, 4>(v.r(), v.g(), v.b(), alpha); });
    return img4;
}
...
inline ImageN<T, 3> image1_to_image3(const ImageN<T, 1>& img1) {
    ...
    std::transform(img1.data().begin(), img1.data().end(), img3.data().begin(),
                   [](const T& v) { return math::Vector<T, 3>(v, v, v); });
    return img3;
}
...
inline ImageN<T, 1> image3_to_image1(const ImageN<T, 3>& img3) {
    ...
    std::transform(img3.data().begin(), img3.data().end(), img1.data().begin(),
                   [](const math::Vector<T, 3>& v) { return (v.r() + v.g() + v.b()) / static_cast<T>(3); });
    return img1;
}
...
inline ImageN<T, 1> image3_to_image1_perceived(const ImageN<T, 3>& img3) {
    ImageN<T, 1> img1(img3.width(), img3.height());
    std::transform(img3.data().begin(), img3.data().end(), img1.data().begin(), [](const math::Vector<T, 3>& v) {
        return static_cast<T>(0.299) * v.r() + static_cast<T>(0.587) * v.g() + static_cast<T>(0.114) * v.b();
    });
    return img1;
}
