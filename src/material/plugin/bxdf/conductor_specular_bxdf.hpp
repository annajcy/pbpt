...
public:
    ConductorSpecularBxDF(const radiometry::SampledSpectrum<T, N>& eta, const radiometry::SampledSpectrum<T, N>& k) : m_eta(eta), m_k(k) {}
...
private:
    ...
    radiometry::SampledSpectrum<T, N> f_impl(
        ...
        // due to the delta distribution, f is zero because the probability of sampling the exact reflection direction is zero
    ...
    std::optional<BxDFSampleRecord<T, N>> sample_f_impl(
        ...
        record.wi = wi;
        record.pdf = 1.0; // delta distribution in reflection direction, pdf is 1 under the condition of hitting the exact direction
    ...
    T pdf_impl(
        ...
        // due to the delta distribution, the PDF is zero because the probability of sampling the exact reflection direction is zero
        return 0;
    }
};
