...
public:
    ...
    UniformDistributionND(const Point<Float, N>& min_pt, const Point<Float, N>& max_pt) 
        : m_min(min_pt), m_max(max_pt) {
        for (int i = 0; i < N; ++i) {
            if (min_pt[i] >= max_pt[i]) {
                throw std::invalid_argument("Lower bounds must be less than upper bounds in all dimensions");
            }
        }
    }
