 ...
 * @details This file provides a `Quaternion` class that represents rotations in 3D space
 * using quaternions. It is designed to integrate seamlessly with the existing math library
 * ecosystem, providing efficient rotation operations, interpolation, and conversion utilities.
 */
 ...
 * @details Quaternions provide a compact and efficient way to represent rotations in 3D space.
 * They avoid gimbal lock issues and provide smooth interpolation. The quaternion is stored
 * as (w, x, y, z) where w is the scalar part and (x, y, z) is the vector part.
 *
 * Key features:
...
public:
    ...
    /// Get vector part
    constexpr Vector<T, 3> vector() const noexcept {
        return Vector<T, 3>(m_x, m_y, m_z);
    }
    ...
    constexpr Quaternion operator*(const Quaternion& rhs) const noexcept {
        return Quaternion(
            m_w * rhs.m_w - m_x * rhs.m_x - m_y * rhs.m_y - m_z * rhs.m_z,  // w
    ...
    /// Spherical linear interpolation (recommended for smooth rotation interpolation)
