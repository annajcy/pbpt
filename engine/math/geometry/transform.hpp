...
public:
    static constexpr Transform translate(const Vec3& t) noexcept {
        return Transform(Mat4(1, 0, 0, t.x(), 0, 1, 0, t.y(), 0, 0, 1, t.z(), 0, 0, 0, 1));
    }

    static constexpr Transform scale(Float s) noexcept {
        return Transform(Mat4(s, 0, 0, 0, 0, s, 0, 0, 0, 0, s, 0, 0, 0, 0, 1));
    }
    ...
    static constexpr Transform rotate(Float angle_rad, const Vec3& axis) noexcept {
        ...
        // 罗德里格斯旋转公式的矩阵形式
        return Transform(Mat4(c + ax * ax * omc, ax * ay * omc - az * s, ax * az * omc + ay * s, 0,
                              ay * ax * omc + az * s, c + ay * ay * omc, ay * az * omc - ax * s, 0,
                              az * ax * omc - ay * s, az * ay * omc + ax * s, c + az * az * omc, 0, 0, 0, 0, 1));
    }
    ...
    static constexpr Transform look_at(const Pt3& eye, const Pt3& target, const Vec3& up) noexcept {
        ...
        return Transform(Mat4(s.x(), s.y(), s.z(), -s.dot(eye.to_vector()), u.x(), u.y(), u.z(),
    ...
    static constexpr Transform orthographic(Float left, Float right, Float bottom, Float top, Float z_near,
                                            Float z_far) noexcept {
        return Transform(Mat4(2.0 / (right - left), 0, 0, -(right + left) / (right - left), 0, 2.0 / (top - bottom), 0,
                              -(top + bottom) / (top - bottom), 0, 0, 1.0 / (z_far - z_near),
                              -z_near / (z_far - z_near), 0, 0, 0, 1));
    }
...
public:
    ...
    constexpr Transform operator*(const Transform& rhs) const noexcept { return Transform(m_mat * rhs.m_mat); }

    constexpr Pt3 operator*(const Pt3& point) const noexcept { return (m_mat * Homo3(point)).to_point(); }
