...
public:
    ...
    virtual Float estimate(const std::function<Float(const Point<Float, N>&)>& f,
                           const std::shared_ptr<Distribution<N>>&             distribution,
                           const std::shared_ptr<Sampler<N>>& sampler, int sample_count) const = 0;
};

class MonteCarloIntegrator1D : public Integrator<1> {
public:
    Float estimate(const std::function<Float(const Point<Float, 1>&)>& f,
                   const std::shared_ptr<Distribution<1>>& distribution, const std::shared_ptr<Sampler<1>>& sampler,
...
public:
    Float estimate(const std::function<Float(const Point<Float, N>&)>& f,
                   const std::shared_ptr<Distribution<N>>& distribution, const std::shared_ptr<Sampler<N>>& sampler,
                   int sample_count) const override {
        if (sample_count <= 0) {
            throw std::invalid_argument("Sample count must be positive");
        }
